...
struct StoryBrain {
    ...
    mutating func updateNextStoryNumber(choiceText: String){
        if(choiceText == self.getChoice1()){
            storyNumber = self.story[storyNumber].choice1Destination
        }else{
            storyNumber = self.story[storyNumber].choice2Destination
        }
    }
}
